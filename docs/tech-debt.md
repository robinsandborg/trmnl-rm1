...
## Issue #3 recovery changes

The [recovery extension](recovery-and-battery.md) addresses TD-03/04's atomic state, independent restore snapshot, bounded early-cycle recovery and ownership, plus TD-06's pending timer cleanup and cold-boot redraw. New installation metadata captures all-false snapshots before service masking. The RM1 boot geometry reset has an explicit raw-rotation configuration migration. Physical battery thresholds, depletion recovery, wall-charger wake/power-on behavior and endurance remain unverified; provisional defaults must not be presented as measured improvements. Kernel hangs, loss of every backup, read-only/full filesystems and external removal of service wiring remain outside automatic recovery guarantees.
