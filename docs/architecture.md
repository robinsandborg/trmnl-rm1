...
Updated for Phase 3 on 2026-09-14, based on merged Phase 2 commit `54f2fdd` in an isolated worktree. Phase 0 also inspected uncommitted Wi-Fi SDIO and stock-noise masking work in the shared checkout; that work is excluded from this phase and is not part of this map. Proposed extractions live in [the phase plan](refactoring-plan.md); remaining risks live in [technical debt](tech-debt.md).

## Shape and entrypoints

One Go module, `github.com/robinsandborg/rm1-trmnl`, declares Go 1.26 and depends on `golang.org/x/image v0.39.0` for BMP decoding. There are four Go packages: executable `cmd/trmnl-rm1`, CLI/cycle composition in `internal/trmnl`, protocol module `internal/byos`, and image preparation/rendering in `internal/display`. Dependencies flow from the executable to `trmnl`, then to `byos` and `display`. Neither extracted package imports `trmnl`; BYOS uses only the standard library, while display also uses the BMP decoder. There is no Makefile. [GitHub Actions checks](../.github/workflows/checks.yml) run macOS/Linux tests and vet plus ARMv7 executable/test compilation.
