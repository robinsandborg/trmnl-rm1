...
func snapshot(state State) appliance.Snapshot {
	return appliance.Snapshot{StockSyncUnit: state.StockSyncUnit, SyncWasEnabled: state.SyncWasEnabled, XochitlWasEnabled: state.XochitlWasEnabled}
}
...
func runRestoreWithOps(state State, ops applianceOps) error {
	return appliance.Restore(snapshot(state), appliance.RestoreOps{Run: ops.run, Remove: ops.remove, SleepHookDir: ops.detectSleepHookDir})
}
